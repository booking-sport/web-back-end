...
exports.isSelfPlayer = (req, res, next) => {
  ...
  if (playerId != currentPlayerId)
    return next(errorHandler(403, "not this player"));
  next();
};
...
exports.isSelfAdmin = (req, res, next) => {
  ...
  if (managerId != currentManagerId)
    return next(errorHandler(403, "not this manager"));
  next();
};
...
exports.isManagerStadium = async (req, res, next) => {
  ...
  if (!managers.includes(managerId))
    return next(errorHandler(403, "not this stadium manager"));
  next();
};
...
exports.isPlayerOrder = async (req, res, next) => {
  ...
  if (order.player_id != playerId)
    return next(errorHandler(401, "player not own this order"));
  next();
};
...
exports.isManagerOrder = async (req, res, next) => {
  ...
  if (!orderIds.includes(orderId))
    return next(errorHandler(401, "manager not own this order"));
  next();
};
