...
class PlayerController {
  ...
  getStatisticByMonth = async(req,res,next) => {
    try {
        const records = await this.userService.countPlayersByMonth();
  ...
  create = async (req, res, next) => {
    try {
      ...
      const newPlayerId = await this.userService.savePlayer({
        fullName,
        email,
        hashedPassword,
        phoneNumber,
  ...
  login = async (req, res, next) => {
    try {
      ...
      if (!bcryptjs.compareSync(password, player.password))
  ...
  update = async (req, res, next) => {
    try {
      ...
      const player = await this.userService.findPlayerbyId(playerId);
      if (oldPassword && !bcryptjs.compareSync(oldPassword, player.password))
        return next(errorHandler(403, "current password is invalid"));
      if (newPassword) hashedPassword = bcryptjs.hashSync(newPassword);

      const newPlayer = await this.userService.updatePlayer(playerId, {
        fullName,
        hashedPassword,
        phoneNumber,
      });
