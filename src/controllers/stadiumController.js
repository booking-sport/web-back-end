...
class StadiumController {
  ...
  toggleActive = async (req, res, next) => {
    try {
      const stadiumId = req.params.stadiumId;
      const currentStadium = await this.stadiumService.findById(stadiumId);
      const isBanned = currentStadium.isBanned;
      const newStadium = await this.stadiumService.update(stadiumId, {
        isBanned: !isBanned,
      });
      res.status(200).json({ data: newStadium });
    } catch (error) {
