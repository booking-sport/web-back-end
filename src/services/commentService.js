...
class CommentService {
  ...
  findById = async (commentId) => {
    try {
      const basicInfo = await this.db("comments")
        .select("*")
        .where("id", commentId)
        .first();
      const allImages = await this.findImagesForAllComment();
  ...
  countRatingsForAll = async () => {
    try {
      const ratingCnts = await this.db("comments")
        .select("stadium_id", "rate")
        .count("* as count")
        .groupBy("stadium_id", "rate")
      return ratingCnts;
    } catch (error) {
      throw errorHandler(503, error.message);
    }
  ...
  saveComment = async (comment) => {
    try {
      ...
      if (images && images.length > 0)
        await this.saveImages(images, newCommentId);
      return newCommentId;
    } catch (error) {
      throw errorHandler(503, error.message);
    }
