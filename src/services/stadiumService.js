...
class StadiumService {
  ...
  findById = async (stadiumId) => {
    try {
      const basicInfo = await this.db("stadiums")
        .select("*")
        .where("id", stadiumId)
        .first();
  ...
  saveStadium = async (stadium) => {
    try {
      ...
      if (images && images.length > 0)
        await this.saveStadiumImages(images, newStadiumId);
      return newStadiumId;
    } catch (error) {
      throw errorHandler(503, error.message);
    }
  ...
  saveField = async (stadiumId, field) => {
    try {
      const newFieldId = await this.db("fields").insert({
        stadium_id: stadiumId,
        ...field,
      });
      return newFieldId;
  ...
  assignOwner = async (stadiumId, ownerId) => {
    try {
      await this.db("stadiums_managers")
        .where({ stadium_id: stadiumId, role: "owner" })
        .delete();
      await this.db("stadiums_managers").insert({
        stadium_id: stadiumId,
        manager_id: ownerId,
        role: "owner",
      });
